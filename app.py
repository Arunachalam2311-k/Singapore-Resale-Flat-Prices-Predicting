...
st.markdown("<h1 style='text-align: center; color: red;'>Singapore Flat Resale Price Prediction</h1>", unsafe_allow_html=True)
    ...
    if option == "Processed Data":
        ...
        with col3:
            town_mapping ={'ANG MO KIO': 1,'BEDOK': 2,'BISHAN': 3,'BUKIT BATOK': 4,'BUKIT MERAH': 5,
                                ...
                                'HOUGANG': 11,'JURONG EAST': 12,'JURONG WEST': 13,'KALLANG/WHAMPOA': 14,'MARINE PARADE': 15,'QUEENSTOWN': 16,
                                'SENGKANG': 17,'SERANGOON': 18,'TAMPINES': 19,'TOA PAYOH': 20,'WOODLANDS': 21,'YISHUN': 22,'LIM CHU KANG': 23,
        ...
        with col4:
            # Flat Model
            flat_model_mapping = {
                'IMPROVED': 1, 'NEW GENERATION': 2, 'MODEL A': 3, 'STANDARD': 4,
                'SIMPLIFIED': 5, 'MODEL A-MAISONETTE': 6, 'APARTMENT': 7, 'MAISONETTE': 8,
                'TERRACE': 9, '2-ROOM': 10, 'IMPROVED-MAISONETTE': 11, 'MULTI GENERATION': 12,
                'PREMIUM APARTMENT': 13, 'Improved': 14, 'New Generation': 15, 'Model A': 16,
                'Standard': 17, 'Apartment': 18, 'Simplified': 19, 'Model A-Maisonette': 20,
                'Maisonette': 21, 'Multi Generation': 22, 'Adjoined flat': 23, 'Premium Apartment': 24,
                'Terrace': 25, 'Improved-Maisonette': 26, 'Premium Maisonette': 27, '2-room': 28,
            ...
            flat_type_value = flat_model_mapping[flat_model]

            # Input for area in sq.m
            floor_area = st.number_input("**Enter the area (in sq.m)**", value=35.0)
        ...
        with col5:
            ...
            resale_month = st.number_input('**Enter the resale month**', value=12, min_value=1, max_value=12)
            ...
            if st.button('Predict'):
                ...
                missing_cols = set(rf_regressor.feature_names_in_) - set(features_df.columns)
                if missing_cols:
                    st.error(f"Missing columns in input data: {missing_cols}")
                    for col in missing_cols:
                        # Add missing columns with default values (e.g., 0 or mean)
                        features_df[col] = 0  # You may choose a different default value

                # Ensure the feature columns are in the same order as during training
                features_df = features_df[rf_regressor.feature_names_in_]
...
with tab3:
            ...
            st.write('* Overall, this project will be useful in :red[predicting the resale prices of flats and understanding the key factors] that can enhance their value.')
